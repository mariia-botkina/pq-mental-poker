...
def encrypt(pk: int, m: int, G) -> tuple[int, int]:
    ...
    if not G.in_subgroup(pk):
        raise ValueError(
            f"public key {pk} must be an element of the order-{G.q} subgroup"
        )
...
def decrypt(sk: int, ciphertext: tuple[int, int], G) -> int:
    ...
    if not G.in_subgroup(v):
        raise ValueError(
            f"ephemeral public key {v} must be an element of the order-{G.q} subgroup"
        )
    if not G.in_subgroup(c):
        raise ValueError(
            f"masked message {c} must be an element of the order-{G.q} subgroup"
        )
